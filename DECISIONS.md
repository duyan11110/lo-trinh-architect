...
- (2026-09-06) **Chặn (chờ chủ):** `gh` chưa cài và chưa đăng nhập trên máy này (không `GH_TOKEN`/`GITHUB_TOKEN`, không auth interactive được ở phiên headless) → KHÔNG tạo được `duyan11110/don-hang` trên GitHub, KHÔNG mở PR được ở Bước 7. Áp dụng fallback A1: `examples/don-hang` là repo git độc lập tại chỗ (đã có commit: seed stage-0 + `.gitattributes`), thêm vào `.gitignore` của repo mẹ, KHÔNG nested-commit. Việc của chủ: `winget install GitHub.cli` rồi `gh auth login`; sau đó agent sẽ `gh repo create duyan11110/don-hang --public`, push 2 commit hiện có lên remote mới, rồi `git submodule add` đúng theo A1.
